...
if (like.src) {
    like.addEventListener('click',(e) => {
        ...
        if (like.src == `http://localhost:8080/public/additional_images/liked_heart.png`) {
            fetch(`/unlike/${i}`).then((result) => {
                like.src= `http://localhost:8080/public/additional_images/like_heart.png`
                number_like.textContent = `${number - 1}`
            })    
        }
        else {
            fetch(`/like/${i}`).then((result) => {
                like.src= `http://localhost:8080/public/additional_images/liked_heart.png`
                number_like.textContent = `${number + 1}`
            })
        }
    }) 
